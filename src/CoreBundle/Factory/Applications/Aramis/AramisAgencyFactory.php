...
class AramisAgencyFactory extends AbstractFactory
{
    ...
    public function createFromEntity($aramisAgency)
    {
        ...
        foreach ($aramisAgency as  $key => $value) {
            if (!in_array("set" . Inflector::camelize($key), array ("setshortUrl", "setfullUrl", "setopening_text1", "setopening_hours1", "setopening_text2", "setopening_hours2", "setlinks", "setschedules", "setpurchaseCalendar", "setsaleCalendar", "setpurchaseSaleCalendar", "setsalespersons", "setappointementRadius"))) {
                if ($value != "") {
